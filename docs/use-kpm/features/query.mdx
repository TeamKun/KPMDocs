...
このページでは、プラグイン解決クエリ（クエリ、クエリ文字列）について説明しています。
...
これは、インストールコマンドの引数に指定して使用するものです。
...
- `curseforge>https://dev.bukkit.org/projects/example-12345==123456`  
  完全就職クエリの例です。CurseForge(DevBukkit) で解決します。

## GitHub の優越

GitHub で解決する場合、以下の省略記法が有効です。この場合においても、リゾルバ名とバージョンを指定できます。
...
- `Example`  
  ...
  組織名の省略は、予め設定された組織のみ有効です。

  <BelowDocument docId="use-kpm/features/resolver" message={"組織名を省略する設定は以下を参照してください："} small />

  :::

## [バッカス・ナウア記法(BNF)](https://ja.wikipedia.org/wiki/%E3%83%90%E3%83%83%E3%82%AB%E3%82%B9%E3%83%BB%E3%83%8A%E3%82%A6%E3%82%A2%E8%A8%98%E6%B3%95)
...
<RelatedFeatures docNames={["resolver", "alias"]} headerLevel={0} />
