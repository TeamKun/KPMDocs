...
このページでは、 KPM 情報ファイルの概要について説明しています。

---

## 概要

KPM 情報ファイルは、KPM 向けの注釈を与えるファイルのことです。プラグインの jar ファイルに埋め込んで使用します。
例えば、プラグインの動作に必要な依存関係の URL を指定したり、自身をアップグレードするためのクエリを指定したりします。
...
エディタ等でこのスキーマをインポートしてご利用ください。

## KPM 情報ファイルを記述する

KPM 情報ファイルを記述して、プラグインと KPM を連携しましょう！  
...
ビルドの成果物の直下に `kpm.yml` が配置される構成であれば、何でも構いません。
...
:::warning
サーバにある KPM のバージョンがこれよりも低い場合、プラグインの読み込みはブロックされるか、インストール時に警告が表示されます。
また、バージョンの指定は [Semantics Versioning](https://semver.org/lang/ja/) に従ってください。
...
:::tip
バージョン指定の `v` 接頭辞は任意です。
これは、 クエリのバージョンなど、 KPM におけるどのようなバージョンの指定においても同様です。
:::

:::warning

ここで指定された KPM のバージョンより、サーバの KPM が古い場合は、プラグインがインストールされません。  
これは、サーバ環境の保護が目的です。
...
プラグインのアップグレードを自動で行うために、 自身の解決[クエリ](/docs/use-kpm/features/query)を指定できます。
...
プラグインの動作に必要な依存プラグインを解決するために、 依存しているプラグインの解決クエリを指定できます。

:::tip
全ての依存関係の解決クエリを記述する必要はなく、この記述は任意です。
:::
...
<BelowDocument docId={"hook/README"} /> 
...
これを指定すると、プラグインが登録したカスタムレシピを KPM が認識し、アンインストール時等に適切に削除します。
